...
async function starts() {
	const anker = new WAConnection()
	anker.logger.level = 'warn'
	anker.on('qr', () => {
		console.log(color('[','white'), color('!','red'), color(']','white'), color(' Scan the qr code above'))
	})

	fs.existsSync('./anker.json') && anker.loadAuthInfo('./anker.json')
	anker.on('connecting', () => {
		start('2', 'Connecting...')
	})
	anker.on('open', () => {
	...
	await anker.connect({timeoutMs: 30*1000})
        fs.writeFileSync('./anker.json', JSON.stringify(anker.base64EncodedAuthInfo(), null, '\t'))

	anker.on('group-participants-update', async (anu) => {
		if (!welkom.includes(anu.jid)) return
		try {
			const mdata = await anker.groupMetadata(anu.jid)
			console.log(anu)
			if (anu.action == 'add') {
				num = anu.participants[0]
				try {
					ppimg = await anker.getProfilePicture(`${anu.participants[0].split('@')[0]}@c.us`)
				} catch {
					ppimg = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
				}
				teks = `*[ SELAMAT DATANG DI GROUP ${mdata.subject} ]*\n*═══════════════════*\n@${num.split('@')[0]} *INTRO DULU!!!*\n*═══════════════════*\n*Jika Anda Tidak Elit, Tombol Keluar Ada Di Pengaturan*\n*═══════════════════*\n*✤ nama :*\n*✤ umur :*\n*✤ gender :*\n*✤ asal kota :*\n*✤ nama IG :*\n*✤ Ada Doi? :*\n*✤ Nama Mantan? :*\n*═══════════════════*\n*NOTE : SAVE NOMER ADMIN*\n*SEMOGA BETAH*`
				let buff = await getBuffer(ppimg)
				anker.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			} else if (anu.action == 'remove') {
				num = anu.participants[0]
				try {
					ppimg = await anker.getProfilePicture(`${num.split('@')[0]}@c.us`)
				} catch {
					ppimg = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
				}
	...
	anker.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				levelnol: '*LEVEL KAKAK MASIH* 0 -_-',
				error: {
			...
			const totalchat = await anker.chats.all()
			const sender = isGroup ? mek.participant : mek.key.remoteJid
			const groupMetadata = isGroup ? await anker.groupMetadata(from) : ''
			...
			const reply = (teks) => {
				anker.sendMessage(from, teks, text, {quoted:mek})
			}
			const sendMess = (hehe, teks) => {
				anker.sendMessage(hehe, teks, text)
			}
			const mentions = (teks, memberr, id) => {
				(id == null || id == undefined || id == false) ? anker.sendMessage(from, teks.trim(), extendedText, {contextInfo: {"mentionedJid": memberr}}) : anker.sendMessage(from, teks.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJid": memberr}})
			}
                        const sendImage = (teks) => {
		                anker.sendMessage(from, teks, image, {quoted:mek})
		        }
		        const costum = (pesan, tipe, target, target2) => {
			        anker.sendMessage(from, pesan, tipe, {quoted: { key: { fromMe: false, participant: `${target}`, ...(from ? { remoteJid: from } : {}) }, message: { conversation: `${target2}` }}})
			}
		        const sendPtt = (teks) => {
		                anker.sendMessage(from, audio, mp3, {quoted:mek})
		        }
          ...
          const checkLimit = (sender) => {
                let found = false
                    for (let lmt of _limit) {
                        if (lmt.id === sender) {
                            ...
                            if (limitCounts <= 0) return anker.sendMessage(from,`Limit request anda sudah habis\n\n_Note : Limit akan direset setiap jam 21:00!_`, text,{ quoted: mek})
                            anker.sendMessage(from, ind.limitcount(limitCounts), text, { quoted : mek})
                            found = true
                        }
                    }
                    if (found === false) {
           ...
           const isLimit = (sender) =>{
              ...
              if (limits >= limitawal ) {
                  position = true
                    anker.sendMessage(from, ind.limitend(pushname), text, {quoted: mek})
                    return true
              } else {
                _limit
                  position = true
		...
		anker.sendMessage(from, bahasa(prefix, sender), text, {quoted: mek})
                break
               case 'virtex':
               anker.sendMessage(from, virtex(prefix, sender), text, {quoted: mek})
               break
               case 'kodenegara':
               anker.sendMessage(from, negara(prefix, sender), text, {quoted: mek})
               break
				case 'demote':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Tag Member Yang Mau Di Demote')
					...
					if (mentioned.length > 1) {
						...
						anker.groupDemoteAdmin(from, mentioned)
					} else {
						mentions(`Perintah di terima, menurunkan jabatan sebagai admin : @${mentioned[0].split('@')[0]}\n Di Group _*${groupMetadata.subject}*_`, mentioned, true)
						anker.groupDemoteAdmin(from, mentioned)
					}
					break
                                case 'randomhentai':
                                        ...
                                        anker.sendMessage(from, buffer, image, {quoted: mek})
                  ...
                  case 'kicktime':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					setTimeout( () => {
					anker.sendMessage(from, 'Yok Sama" Al-fatihah Buat Yang Keluar', text)
					}, 31000)
					setTimeout( () => {
					reply('Done Min:v')
					}, 30000)
					setTimeout( () => {
					anker.groupRemove(from, mentioned)
					}, 29000)
					setTimeout( () => {
					anker.sendMessage(from, `Bismilah Kick @${mentioned[0].split('@')[0]} aowkwowk`, text) // ur cods
					}, 24000)
					setTimeout( () => {
					anker.sendMessage(from, 'Gw Kick Skrg Ya min', text)
					}, 18000)
					setTimeout( () => {
					anker.sendMessage(from, 'Kick Skrg Min? Apa kasih Waktu Lagi?', text)
                    }, 14000)
					setTimeout( () => {
					anker.sendMessage(from, `Dah Siap Blm Ngab @${mentioned[0].split('@')[0]}`, text)
					}, 8000)
					setTimeout( () => {
					anker.sendMessage(from, 'Otw Kick Min Tapi Gw Kasih Waktu Dlu Ya Min', text)
					}, 2500)
					setTimeout( () => {
                  ...
                  case 'promote':
					...
					if (mentioned.length > 1) {
						...
						mentions(teks, mentioned, true)
						anker.groupMakeAdmin(from, mentioned)
					} else {
  ...
  anker.updatePresence(from, Presence.composing) 
    ...
    anker.sendMessage(from, options, text, { quoted: mek } )
				break
			...
			case 'quotes':
                ...
                if (!isRegister) return reply(mess.only.daftarB)
                if (isLimit(sender)) return reply(ind.limitend(pusname))
                anker.sendMessage(from, data, image, {quoted: mek, caption: body.slice(8)})
                await limitAdd(sender)
                break
                case 'fml':
              ...
              case 'owner':
                case 'creator':
                  anker.sendMessage(from, {displayname: "Jeff", vcard: vcard}, MessageType.contact, { quoted: mek})
               anker.sendMessage(from, 'Nih nomor ownerku kak, save ya kak nanti di save balik',MessageType.text, { quoted: mek} )
                break
	case 'hidetag':
                anker.updatePresence(from, Presence.composing) 
                if (!isRegister) return reply(mess.only.daftarB)
                if (!isGroup) return reply(mess.only.group)
                teks = body.slice(9)
                group = await anker.groupMetadata(from);
              ...
              await anker.sendMessage(from, options, text)
               break
                                case 'tiktokstalk':
					try {
						if (args.length < 1) return anker.sendMessage(from, 'Usernamenya mana kak? ', text, {quoted: mek})
                                                if (!isRegister) return reply(mess.only.daftarB)
                                                if (isLimit(sender)) return reply(ind.limitend(pusname))
						...
						anker.sendMessage(from, buffer, image, {quoted: mek, caption: teks})
                                                await limitAdd(sender)
					} catch (e) {
		...
		case 'infonomor':
               anker.updatePresence(from, Presence.composing) 
                ...
                break
                   ...
                   anker.sendMessage(from, hasil, image, {quoted: mek, caption: `Hasil Dari *${body.slice(5)}*`})
                   await limitAdd(sender)
                   break
                   case 'covidcountry':
                   anker.updatePresence(from, Presence.composing) 
				...
				case 'ytmp3':
					...
					anker.sendMessage(from, thumb, image, {quoted: mek, caption: teks})
					buffer = await getBuffer(anu.result)
					anker.sendMessage(from, buffer, audio, {mimetype: 'audio/mp4', filename: `${anu.title}.mp3`, quoted: mek})
                                        await limitAdd(sender)
					break
				case 'ytmp4':
					if (args.length < 1) return reply('Urlnya mana um?')
					...
					anker.sendMessage(from, thumb, image, {quoted: mek, caption: teks})
					buffer = await getBuffer(anu.result)
					anker.sendMessage(from, buffer, video, {mimetype: 'video/mp4', filename: `${anu.title}.mp4`, quoted: mek})
                                        await limitAdd(sender)
					break
				case 'trendtwit':
					anker.updatePresence(from, Presence.composing) 
				...
				case 'testime':
					setTimeout( () => {
					anker.sendMessage(from, 'Waktu habis:v', text) // ur cods
					}, 10000) // 1000 = 1s,
					setTimeout( () => {
					anker.sendMessage(from, '5 Detik lagi', text) // ur cods
					}, 5000) // 1000 = 1s,
					setTimeout( () => {
					anker.sendMessage(from, '10 Detik lagi', text) // ur cods
					}, 0) // 1000 = 1s,
					break
				/*case 'semoji':
					...
					exec(`wget ${anu.result} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
						...
						anker.sendMessage(from, buffer, sticker)
						fs.unlinkSync(rano)
					})
					break*/
				case 'nulis':
				case 'tulis':
					...
					anker.sendMessage(from, buff, image, {quoted: mek, caption: mess.success})
                                        await limitAdd(sender)
					break
	case 'kbbi':
            anker.updatePresence(from, Presence.composing) 
	      ...
	      tels = body.slice(6)
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
                ...
                anker.sendMessage(from, lagu, audio, {mimetype: 'audio/mp4', filename: `${data.result.title}.mp3`, quoted: mek})
                await limitAdd(sender)
                break
				case 'blocklist':
					...
					anker.sendMessage(from, teks.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJid": blocked}})
					break
                   case 'chatlist':
					anker.updatePresence(from, Presence.composing)  
					...
					anker.sendMessage(from, teks.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJid": totalchat}})
					break
				case 'animecry':
					ranp = getRandom('.gif')
					rano = getRandom('.webp')
					...
					exec(`wget ${anu.result} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=15 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
						...
						if (err) return reply(mess.error.stick)
						buffer = fs.readFileSync(rano)
						anker.sendMessage(from, buffer, sticker, {quoted: mek})
						fs.unlinkSync(rano)
					})
					break
				case 'neonime':
                ...
                if (isLimit(sender)) return reply(ind.limitend(pusname))
                anker.sendMessage(from, data, image, {quoted: mek, caption: body.slice(7)})
                await limitAdd(sender)
                break
				case 'tts':
				   anker.updatePresence(from, Presence.recording) 
				   if (args.length < 1) return anker.sendMessage(from, 'Kode bahasanya mana om?', text, {quoted: mek})
                                   if (!isRegister) return reply(mess.only.daftarB)
                                   if (isLimit(sender)) return reply(ind.limitend(pusname))
					const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return anker.sendMessage(from, 'Textnya mana om', text, {quoted: mek})
					dtt = body.slice(8)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 600
					...
					: gtts.save(ranm, dtt, function() {
						exec(`ffmpeg -i ${ranm} -ar 48000 -vn -c:a libopus ${rano}`, (err) => {
							...
							anker.sendMessage(from, buff, audio, {quoted: mek, ptt:true})
							fs.unlinkSync(rano)
						})
					})
                                        await limitAdd(sender)
				...
				case 'pokemon':
					...
					anker.sendMessage(from, pok, image, { quoted: mek })
                                        await limitAdd(sender)
					break
                case 'pinterest':
                                        tels = body.slice(11)
					...
					nimek =  n[Math.floor(Math.random() * n.length)];
					pok = await getBuffer(nimek)
					anker.sendMessage(from, pok, image, { quoted: mek, caption: `*PINTEREST*\n\*Hasil Pencarian* : *${tels}*`})
                                        await limitAdd(sender)
					break
				case 'setprefix':
					anker.updatePresence(from, Presence.composing) 
				...
				case 'block':
					...
					if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply(mess.only.ownerB)
					anker.blockUser (`${body.slice(8)}@c.us`, "add")
					anker.sendMessage(from, `perintah Diterima, memblokir ${body.slice(8)}@c.us`, text)
					break
				case 'hilih':
					anker.updatePresence(from, Presence.composing) 
				...
				case 'send':
					...
					anker.sendMessage(nomor+'@s.whatsapp.net', pesan, text)
					break
					case 'quotesnime':
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				if (!isQuotedImage) return reply(`Kirim gambar dengan caption ${prefix}setbotpp atau tag gambar yang sudah dikirim`)
					...
					await anker.updateProfilePicture(botNumber, media)
				...
				case 'bc':
					...
					anu = await anker.chats.all()
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
						buff = await anker.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							anker.sendMessage(_.jid, buff, image, {caption: `*『 BROADCAST 』*\n\n${body.slice(4)}`})
						}
						reply('')
					} else {
						for (let _ of anu) {
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of groupMembers) {
							anker.sendMessage(_.jid, buff, image, {caption: `*『 BC GROUP 』*\n*Group* : ${groupName}\n\n${body.slice(6)}`})
						}
						reply('')
					} else {
						for (let _ of groupMembers) {
		...
		case 'tahta':
                ...
                tahta = `${body.slice(7)}`
                  ...
                  break  
                    ...
                    if (!isGroupAdmins) return reply(mess.only.admin)
                     setTimeout( () => {
					anker.groupLeave (from) 
					}, 2000)
                     setTimeout( () => {
					anker.updatePresence(from, Presence.composing) 
					anker.sendMessage(from, 'Sayonara👋', text) // ur cods
					}, 0)
                     break
			...
			case 'igstalk':
                    ...
                    anker.sendMessage(from, buffer, image, {quoted: mek, caption: hasil})
                    await limitAdd(sender)
                    break
                    case 'ownergrup':
				  case 'ownergroup':
          ...
          contextInfo: { mentionedJid: [from] }
           ...
           case 'quran':
					...
					anker.sendMessage(from, quran, text, {quoted: mek})
					break
           case 'nekonime':
           ...
           anker.sendMessage(from, hasil, image, {quoted: mek})
           await limitAdd(sender)
           break
				case 'neko':
                                        if (!isRegister) return reply(mess.only.daftarB)
				...
				case 'kick':
					...
					if (mentioned.length > 1) {
						...
						anker.groupRemove(from, mentioned)
					anker.sendMessage(mentioned, 'yahaha Lu kekick😂', text)
					}
					break
				case 'exe':
	              anker.updatePresence(from, Presence.composing) 
	              if (!isOwner) return reply(mess.only.ownerB)
	               ...
	               exec(cmd, (err, stdout) => {
		           if(err) return anker.sendMessage(from, "Command Salah", text, { quoted: mek })
		           if (stdout) {
			       anker.sendMessage(from, stdout, text, { quoted: mek })
		           }
	           })
                  break
                 case 'linkgroup':
				...
				case 'linkgc':
				    ...
				    if (!isGroup) return reply(mess.only.group)
                                     if (!isRegister) return reply(mess.only.daftarB)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					linkgc = await anker.groupInviteCode (from)
					yeh = `https://chat.whatsapp.com/${linkgc}\n\nLink Group *${groupName}*`
					anker.sendMessage(from, yeh, text, {quoted: mek, detectLinks: false})
					break
                case 'qrcode':
                if (!isRegister) return reply(mess.only.daftarB)
                if (isLimit(sender)) return reply(ind.limitend(pusname))
                buff = await getBuffer(`https://api.qrserver.com/v1/create-qr-code/?data=${body.slice(8)}&size=1080%C3%971080`)
				anker.sendMessage(from, buff, image, {quoted: mek})
                                await limitAdd(sender)
				break
				case 'ocr':
					...
					break
                     ...
                     const bug = body.slice(5)
                      if (pesan.length > 300) return anker.sendMessage(from, 'Maaf Teks Terlalu Panjang, Maksimal 300 Teks', msgType.text, {quoted: mek})
                    ...
                    anker.sendMessage(NomerOwner, options, text, {quoted: mek})
	    ...
	    case 'kapankah':
			...
			case 'closegc':
				...
				case 'toimg':
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						buffer = fs.readFileSync(ran)
						anker.sendMessage(from, buffer, audio, {mimetype: 'audio/mp4', quoted: mek})
						fs.unlinkSync(ran)
					})
					break
	          ...
	          if (!isRegister) return reply(mess.only.daftarB)
               ...
               if (anu.error) return reply(anu.error)
                 infomp3 = `*Lagu Ditemukan!!!*\nJudul : ${anu.result.title}\nSource : ${anu.result.source}\nUkuran : ${anu.result.size}\n\n*TUNGGU SEBENTAR LAGI DIKIRIM MOHON JANGAN SPAM YA AJG*`
                ...
                break
                   ...
                   anker.sendMessage(from, hasil, text, {quoted: mek})
                   await limitAdd(sender)
                   break
                              case 'game':
					...
					setTimeout( () => {
					anker.sendMessage(from, '*➣ Jawaban :* '+anu.result.jawaban+'\n'+anu.result.desk, text, {quoted: mek}) // ur cods
					}, 30000) // 1000 = 1s,
					setTimeout( () => {
					anker.sendMessage(from, '_10 Detik lagiâ€¦_', text) // ur cods
					}, 20000) // 1000 = 1s,
					setTimeout( () => {
					anker.sendMessage(from, '_20 Detik lagi_â€¦', text) // ur cods
					}, 10000) // 1000 = 1s,
					setTimeout( () => {
					anker.sendMessage(from, '_30 Detik lagi_â€¦', text) // ur cods
					}, 1000) // 1000 = 1s,
					setTimeout( () => {
					anker.sendMessage(from, anu.result.soal, text, { quoted: mek }) // ur cods
					}, 0) // 1000 = 1s,
					break
                                case 'welcome':
					if (!isGroup) return reply(mess.only.group)
					...
					if (Number(args[0]) === 1) {
						...
						reply('❰ ☑️ ❱ SUCCSESS  mengaktifkan fitur welcome di group ini')
					} else if (Number(args[0]) === 0) {
						welkom.splice(from, disable)
						fs.writeFileSync('./database/json/welkom.json', JSON.stringify(welkom))
						reply('❰ ☑️ ❱ SUCCSESS  menonaktifkan fitur welcome di group ini')
					} else {
					...
					anker.sendMessage(from, 'fakta : '+ keh, { quoted: mek })
                                        await limitAdd(sender)
					break
                                case 'water':
				...
				case 'firetext':
					...
					anker.sendMessage(from, 'Pertanyaan : *'+ganteng+'*\n\nJawaban : '+ teng+'%', text, { quoted: mek })
					break
					case 'cantikcek':
					if (isRegister) return reply(mess.only.daftarB)
					cantik = body.slice(1)
					...
					anker.sendMessage(from, 'Pertanyaan : *'+cantik+'*\n\nJawaban : '+ tik+'%', text, { quoted: mek })
					break
				case 'watak':
				if (isRegister) return reply(mess.only.daftarB)
                                if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					anker.sendMessage(from, 'Pertanyaan : *'+watak+'*\n\nJawaban : '+ tak, text, { quoted: mek })
                                        await limitAdd(sender)
				    break
				case 'hobby':
				if (isRegister) return reply(mess.only.daftarB)
					...
					anker.sendMessage(from, 'Pertanyaan : *'+hobby+'*\n\nJawaban : '+ by, text, { quoted: mek })
					break
                                case 'nsfwneko':
				    try{
						...
						anker.sendMessage(from, buffer, image, {quoted: mek, caption: 'mesum'})
                                                await limitAdd(sender)
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('❰❗❱ *ERROR* ❰❗❱')
				    ...
				    try{
						...
						buffer = await getBuffer(res.result)
                                                if (!isRegister) return reply(mess.only.daftarB)
                                                if (isLimit(sender)) return reply(ind.limitend(pusname))
						anker.sendMessage(from, buffer, image, {quoted: mek, caption: 'Nich'})
                                                await limitAdd(sender)
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('❰❗❱ *ERROR* ❰❗❱')
				...
				case 'logowolf':
					...
					if (Number(args[0]) === 1) {
						...
						reply('❰ ☑️ ❱ SUCCSESS  mengaktifkan fitur nsfw di group ini')
					} else if (Number(args[0]) === 0) {
						nsfw.splice(from, 1)
						fs.writeFileSync('./database/json/nsfw.json', JSON.stringify(nsfw))
						reply('❰ ☑️ ❱ SUCCSESS  menonaktifkan fitur nsfw di group ini')
					} else {
						reply('ketik 1 untuk mengaktifkan, 0 untuk menonaktifkan fitur')
					}
					break	
				...
				case 'logowolf2':
					...
					if (!isGroup)return reply(mess.only.group)
                                        if (!isRegister) return reply(mess.only.daftarB)
					if (!isGroupAdmins)return reply(mess.only.admin)
					anker.deleteMessage(from, { id: mek.message.extendedTextMessage.contextInfo.stanzaId, remoteJid: from, fromMe: true })
					break
                                case 'phlogo':
					var gh = body.slice(7)
					var gbl1 = gh.split("|")[0];
                ...
                case 'truth':
					...
					anker.sendMessage(from, truteh, image, { caption: '*Truth*\n\n'+ ttrth, quoted: mek })
					break
                                case 'dare':
					...
					anker.sendMessage(from, tod, image, { quoted: mek, caption: '*Dare*\n\n'+ der })
					break	
                case 'level':
               ...
               anker.sendMessage(from, resul, text, { quoted: mek})
                .catch(async (err) => {
                        console.error(err)
                        await reply(`Error!\n${err}`)
                    })
            break
				...
				mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					...
					var bot = gh.split("|")[2];
					anker.sendMessage(from, `${bot}`, text, {quoted: { key: { fromMe: false, participant: `${mentioned}`, ...(from ? { remoteJid: from } : {}) }, message: { conversation: `${target}` }}})
					break
            case 'leveling':
                if (!isGroup) return reply(mess.only.group)
                if (!isGroupAdmins) return reply(mess.only.admin)
            ...
            break
                                ...
                                case 'nsfwtrap':
                                        try{
                                                ...
                                                buffer = await getBuffer(res.result)
                                                anker.sendMessage(from, buffer, image, {quoted: mek, caption: 'Nih gambarnya kak...'})
                                                await limitAdd(sender)
                                        } catch (e) {
                                                console.log(`*Error* :`, color(e,'red'))
                                                reply('❰❗❱ *ERROR* ❰❗❱')
			   	        ...
			   	        if (!isRegister) return reply(mess.only.userB)
                                        ...
                                        anker.updatePresence(from, Presence.composing) 
				        uptime = process.uptime()
                                        anker.sendMessage(from, `Speed: *${latensi.toFixed(4)} _Second_*\nDevice: *Black Shark 3*\nRAM: *8/128*\nData: *Telkomsel*\nJaringan: *4G*\nStatus: *Gak Di Cass*`, text, { quoted: mek})
                                        break
                                case 'neonlogo':
                                        var gh = body.slice(9)
                                        var teks1 = gh.split("|")[0];
                                        ...
                                        buffer = await getBuffer(anu.result)
                                        anker.sendMessage(from, buffer, image, {quoted: mek, caption: 'Nih kak logonya...'})
                                        await limitAdd(sender)
                                        break
                                case 'neonlogo2':
                                ...
                                case 'jsholat':
                                        ...
                                        mbteks = `「 *JAM SHALAT* 」\n ➣ Daerah : ${loc} \n ➣ Ashar : ${anu.Ashar} \n ➣ Dhuha : ${anu.Dhuha} \n ➣ Dzuhur : ${anu.Dzuhur} \n ➣ Imsyak : ${anu.Imsyak} \n ➣ Isya : ${anu.Isya} \n ➣ Maghrib : ${anu.Maghrib} \n ➣ Subuh : ${anu.Subuh}`
                                        anker.sendMessage(from, mbteks, text)
                                        break
                                case 'jokerlogo':
                                        var gh = body.slice(10)
                                        var teks1 = gh.split("|")[0];
			       ...
			       reply(anu.result.jadwalTV)
					break*/
                                case 'afk':
                                        ...
                                        anker.sendMessage(from, tag, text, {quoted: mek})
                                        break
                                case 'shadow':
                                        var gh = body.slice(7)
                                        var teks1 = gh.split("|")[0];
                                        ...
                                        buffer = await getBuffer(anu.result)
                                        anker.sendMessage(from, buffer, image, {quoted: mek, caption: 'Nih kak gambarnya...'})
                                        await limitAdd(sender)
                                        break
                                case 'burnpaper':
                                        var gh = body.slice(10)
                                ...
                                case 'randomkpop':
                                        ...
                                        buffer = await getBuffer(anu.result)
                                        anker.sendMessage(from, buffer, image, {quoted: mek, caption: 'Nih kpopnya kak...'})
                                        await limitAdd(sender)
                                        break
                                case 'fml2':
				...
				case 'ttp':
					...
					exec(`wget ${anu.result} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
						fs.unlinkSync(ranp)
						if (err) return reply(mess.error.stick)
						anker.sendMessage(from, fs.readFileSync(rano), sticker, {quoted: mek})
						fs.unlinkSync(rano)
					})
                                        await limitAdd(sender)
					break
					...
					anu = await anker.chats.all()
					anker.setMaxListeners(25)
					for (let _ of anu) {
						anker.deleteChat(_.jid)
					}
				...
				case 'clone':
					...
					try {
						pp = await anker.getProfilePicture(id)
						buffer = await getBuffer(pp)
						anker.updateProfilePicture(botNumber, buffer)
					...
					break
                                case 'pubglogo':
                                        ...
                                        buffer = await getBuffer(anu.result)
                                        anker.sendMessage(from, buffer, image, {quoted: mek, caption: 'Nih logonya kak...'})
                                        await limitAdd(sender)
                                        break
                                case 'herrypotter':
                                case 'harrypotter':
                                ...
                                case 'event':
                                        ...
                                        if (Number(args[0]) === 1) {
                                                ...
                                                reply('*❰ ☑️ ❱ SUCCSESS  Mengaktifkan EVENT di group ini*')
                                        } else if (Number(args[0]) === 0) {
                                                event.splice(from, 1)
                                                fs.writeFileSync('./database/json/event.json', JSON.stringify(event))
                                                reply('*❰ ☑️ ❱ SUCCSESS  Menonaktifkan EVENT di group ini*')
                                        } else {
                                                reply(ind.satukos())
                                        }
                                        break
			                ...
			                if (!isBotGroupAdmins) return reply(ind.badmin())
                                        anker.groupUpdateSubject(from, `${body.slice(9)}`)
                                        anker.sendMessage(from, 'Succes, Ganti Nama Grup', text, {quoted: mek})
				        break
                                case 'setdesc':
                                        if (!isGroup) return reply(ind.groupo())
			                if (!isGroupAdmins) return reply(ind.admin())
			         	if (!isBotGroupAdmins) return reply(ind.badmin())
                                        anker.groupUpdateDescription(from, `${body.slice(9)}`)
                                        anker.sendMessage(from, 'Succes, Ganti Deskripsi Grup', text, {quoted: mek})
					break*/
                                case 'translate':
                                        aruga = body.slice(10)
                                        lang = aruga.split("|")[0];
                                        ...
                                        anker.sendMessage(from, arteks, text)
                                        await limitAdd(sender)
                                        break
                                case 'tafsirmimpi':
                                        aruga = body.slice(12)
                                ...
                                case 'tagme':
                                        ...
                                        anker.sendMessage(from, tagme, text, {quoted: mek})
                                        break
                                case 'ip': // masih testing
                                        ...
                                        anker.sendMessage(from, teks, text)
                                        await limitAdd(sender)
                                        break
                                case 'happymod':
                                        toby = body.slice(10)
                                ...
                                case 'say':
                                        ...
                                        saying = teks
                                        anker.sendMessage(from, saying, text)
                                        break
			        case 'wait':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
                                        if (!isRegister) return reply(mess.only.daftarB)
						reply(mess.wait)
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
						media = await anker.downloadMediaMessage(encmedia)
						await wait(media).then(res => {
         						anker.sendMessage(from, res.video, video, {quoted: mek, caption: res.teks.trim()})
						}).catch(err => {
							reply(err)
		      				})
					} else {
